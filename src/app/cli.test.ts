...
describe("resolveEntryStep", () => {
  const testWorkflow = Workflow.fromJson({
    id: "test",
    name: "Test",
  ...
  it("returns the specified step with --start", () => {
    const result = resolveEntryStep(testWorkflow, asStepId("step-2"));
    expect(result.error).toBeUndefined();
    expect(result.stepId).toBe("step-2");
  });

  it("returns last step when specified", () => {
    const result = resolveEntryStep(testWorkflow, asStepId("step-3"));
    expect(result.error).toBeUndefined();
    expect(result.stepId).toBe("step-3");
  });

  it("returns error when --start step does not exist", () => {
    const result = resolveEntryStep(testWorkflow, asStepId("step-99"));
    expect(result.error).toBeDefined();
  ...
  it("respects exact step id match", () => {
    const result = resolveEntryStep(testWorkflow, asStepId("step-1"));
    expect(result.error).toBeUndefined();
    expect(result.stepId).toBe("step-1");
  });

  it("does not match partial step ids", () => {
    const result = resolveEntryStep(testWorkflow, asStepId("step"));
    expect(result.error).toBeDefined();
    expect(result.stepId).toBe("");
  });
