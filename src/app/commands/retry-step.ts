...
import { parseRetryStepArgs, USAGE } from "../cli.js";
import { mapDaemonError } from "./_errors.js";

export interface RetryStepDeps {
...
export async function run(
  ...
  const parsed = parseRetryStepArgs(argv);
  if (!parsed.ok) {
    stderr.write(`workflow-runner: ${parsed.error}\n${USAGE.retryStep}\n`);
    return 1;
  }
  if ("help" in parsed) {
    stdout.write(`${USAGE.retryStep}\n`);
    return 0;
  }
  ...
  try {
    const result = await client.call("run.retryStep", {
      runId: asRunId(runIdInput),
    });
    stdout.write(
      `↻ retrying ${result.resumedStepId} — LLM output may differ from the previous attempt\n`,
    );
    return 0;
  } catch (err) {
